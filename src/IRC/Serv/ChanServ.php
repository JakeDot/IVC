...
class ChanServ extends IrcObject
{
    ...
    public static function normalizeChannelName(string $channel): string
    {
        ...
        if (!str_starts_with($channel, '#') && !preg_match('^\w+', $channel)) {
            $channel = '#' . $channel;
        }
        return $channel;
    }
    ...
    public static function checkAccess(string $target, ?string $user = null): array
    {
        ...
        if (str_starts_with($channel, '#') || str_starts_with($channel, '&')) {
            ...
            if ($chanModel !== null) {
                ...
                if (!empty($chanFlags['r'])) $isRegisteredOnly = true;
                if (!empty($chanFlags['i'])) $isIdentifiedOnly = true;
                if (!empty($chanFlags['A'])) $isAdminOnly      = true;
                if (!empty($chanFlags['O'])) $isOpOnly         = true;
                if (!empty($chanFlags['N'])) $isNetAdminOnly   = true;
            }
        }
        ...
        if (!empty($parsed['subobjects'])) {
            foreach ($parsed['subobjects'] as $sub) {
                if (!empty($sub['mode_flags']['r'])) $isRegisteredOnly = true;
                if (!empty($sub['mode_flags']['i'])) $isIdentifiedOnly = true;
            }
        }
